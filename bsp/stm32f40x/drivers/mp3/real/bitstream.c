...
int UnpackFrameHeader(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
	...
	int verIdx;
	FrameHeader *fh;//数据帧头（4字节的数据）

	/* validate pointers and sync word */
	if (!mp3DecInfo || !mp3DecInfo->FrameHeaderPS || (buf[0] & SYNCWORDH) != SYNCWORDH || (buf[1] & SYNCWORDL) != SYNCWORDL)//xqy
		return -1;
	...
	verIdx =         (buf[1] >> 3) & 0x03;
	fh->ver =        (MPEGVersion)( verIdx == 0 ? MPEG25 : ((verIdx & 0x01) ? MPEG1 : MPEG2) );//MPEG1
	fh->layer = 4 - ((buf[1] >> 1) & 0x03);/* easy mapping of index to layer number, 4 = error */
	fh->crc =   1 - ((buf[1] >> 0) & 0x01);//
	fh->brIdx =      (buf[2] >> 4) & 0x0f;//比特率，单位时间内传送的数据位
	fh->srIdx =      (buf[2] >> 2) & 0x03;//采样频率
	fh->paddingBit = (buf[2] >> 1) & 0x01;//帧长调节，填充标志
	fh->privateBit = (buf[2] >> 0) & 0x01;//保留位，没用。
	fh->sMode =      (StereoMode)((buf[3] >> 6) & 0x03);      /* maps to correct enum (see definition) *///立体声，单声道，双声道等    
	fh->modeExt =    (buf[3] >> 4) & 0x03;//当声道为0x01时才用到
	fh->copyFlag =   (buf[3] >> 3) & 0x01;//版权，文件是否合法
	fh->origFlag =   (buf[3] >> 2) & 0x01;//是否原版
	fh->emphasis =   (buf[3] >> 0) & 0x03;//不用，保留
	...
	mp3DecInfo->nChans = (fh->sMode == Mono ? 1 : 2);//判断几声道
	mp3DecInfo->samprate = samplerateTab[fh->ver][fh->srIdx];//采样频率
	mp3DecInfo->nGrans = (fh->ver == MPEG1 ? NGRANS_MPEG1 : NGRANS_MPEG2);//不懂
	mp3DecInfo->nGranSamps = ((int)samplesPerFrameTab[fh->ver][fh->layer - 1]) / mp3DecInfo->nGrans;//每帧采样次数/这个是不是采样的位数？？12位？，一个颗粒采样次数576
	 ...
	 */
	if (fh->brIdx) {
		mp3DecInfo->bitrate = ((int)bitrateTab[fh->ver][fh->layer - 1][fh->brIdx]) * 1000;//获取比特率
	
		/* nSlots = total frame bytes (from table) - sideInfo bytes - header - CRC (if present) + pad (if present) */
		/* nSlots =总帧数字节（从表格） - sideInfo字节 - 标题 - CRC（如果存在）+ pad（如果存在）*/
		mp3DecInfo->nSlots = (int)slotTab[fh->ver][fh->srIdx][fh->brIdx] - 
			(int)sideBytesTab[fh->ver][(fh->sMode == Mono ? 0 : 1)] - 
			4 - (fh->crc ? 2 : 0) + (fh->paddingBit ? 1 : 0);//减4是因为这是数据头帧，每个数据帧都是4字节的头
			//这是计算一帧中的真正需要转成PCM数据的字节数
	}
...
{
	int gr, ch, bd, nBytes;
	BitStreamInfo bitStreamInfo, *bsi;//位流信息
	FrameHeader *fh;
	SideInfo *si;
	SideInfoSub *sis;
	...
	bsi = &bitStreamInfo;
	if (fh->ver == MPEG1) //类似于解析帧头信息一样，按位解析xqy
	{
		/* MPEG 1 */
		nBytes = (fh->sMode == Mono ? SIBYTES_MPEG1_MONO : SIBYTES_MPEG1_STEREO);//立体声
		SetBitstreamPointer(bsi, nBytes, buf);
		si->mainDataBegin = GetBits(bsi, 9);//获取开始的9位数据就是mainDataBegin的值
		...
		for (ch = 0; ch < mp3DecInfo->nChans; ch++)
		{
			...
			{
				si->scfsi[ch][bd] = GetBits(bsi, 1);//缩放因子选择信息
			}
	    }
	} 
	else 
	...
	for(gr =0; gr < mp3DecInfo->nGrans; gr++) //两个颗粒
	{
		for (ch = 0; ch < mp3DecInfo->nChans; ch++) //两个声道
		{
	...
	ASSERT(nBytes == CalcBitsUsed(bsi, buf, 0) >> 3);//xqy貌似是计算该边信息已经使用的个字节数，便于偏移到main_data
