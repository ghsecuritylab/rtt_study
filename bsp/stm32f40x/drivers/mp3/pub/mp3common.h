...
typedef struct _MP3DecInfo {
	...
	void *FrameHeaderPS;//帧头信息指针
	void *SideInfoPS;   //边信息指针
	void *ScaleFactorInfoPS;//比例因子指针
	void *HuffmanInfoPS;    //霍夫曼变化指针
	void *DequantInfoPS;    //逆量化指针
	void *IMDCTInfoPS;      //IMDCT变换指针
	void *SubbandInfoPS;    //子带合成指针

	/* buffer which must be large enough to hold largest possible main_data section */
	unsigned char mainBuf[MAINBUF_SIZE];//储存主信息的全局buffer
