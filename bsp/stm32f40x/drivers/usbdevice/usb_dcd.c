              ...
              USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  USB_OTG_SelectCore (pdev , coreID);//设置端点地址和size的函数，初始化所有寄存器的地址
  
  pdev->dev.device_status = USB_OTG_DEFAULT;
  pdev->dev.device_address = 0;
  
  /* Init ep structure */
  for (i = 0; i < pdev->cfg.dev_endpoints ; i++)//IN端点
  {
    ep = &pdev->dev.in_ep[i];
    /* Init ep structure */
    ep->is_in = 1;
  ...
  for (i = 0; i < pdev->cfg.dev_endpoints; i++)//OUT端点
  {
  ...
  USB_OTG_DisableGlobalInt(pdev);//屏蔽中断
  //根据usb编程操作模式来编写
  /*Init the Core (common init.) */
  USB_OTG_CoreInit(pdev);//打开复位等中断在该函数//模块初始化
  ...
  /* Init Device */
  USB_OTG_CoreInitDev(pdev);//初始化设备
  
  
  /* Enable USB Global interrupt */
  USB_OTG_EnableGlobalInt(pdev);//打开中断
}
