...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsDeviceMode(pdev)) /* ensure that we are in device mode */
  {
    ...
    if (gintr_status.b.outepintr)//out中断
    {
      //rt_kprintf("out\r\n");
      retval |= DCD_HandleOutEP_ISR(pdev);
    }    
    
    if (gintr_status.b.inepint)//in事务中断
    {   
        //rt_kprintf("in\n");
      retval |= DCD_HandleInEP_ISR(pdev);
    }
    
    if (gintr_status.b.modemismatch)//模式不匹配中断
    {
    ...
    if (gintr_status.b.wkupintr)//唤醒中断
    {
        //printf("唤醒中断\r\n");
      retval |= DCD_HandleResume_ISR(pdev);
    }
    
    if (gintr_status.b.usbsuspend)//挂起中断
    {
        //printf("挂起中断\r\n");
      retval |= DCD_HandleUSBSuspend_ISR(pdev);
    }
    if (gintr_status.b.sofintr)//起始帧中断
    {
        //printf("起始帧中断\r\n");
    ...
    if (gintr_status.b.rxstsqlvl)//有数据等待处理
    {
        //rt_kprintf("2\r\n");
    ...
    if (gintr_status.b.usbreset)//复位
    {
      rt_kprintf("复位\r\n");
      retval |= DCD_HandleUsbReset_ISR(pdev);
      
    }
    if (gintr_status.b.enumdone)//枚举完成
    {
...
static uint32_t DCD_OTG_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (gotgint.b.sesenddet)
  {
      ...
      HAL_PCD_DisconnectCallback(pdev);
  }
  /* Clear OTG interrupt */
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) /* In ITR */
    {
      ...
      if ( diepint.b.xfercompl )
      {
        ...
        HAL_PCD_DataInStageCallback(pdev, epnum);
        if (pdev->cfg.dma_enable == 1)
        {
      ...
      if ( diepint.b.timeout )
      {
        //printf("diepint.b.timeout\r\n");
        CLEAR_IN_EP_INTR(epnum, timeout);
      }
      if (diepint.b.intktxfemp)
      {
        //printf("diepint.b.intktxfemp\r\n");
...
static uint32_t DCD_HandleOutEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  ep_intr = USB_OTG_ReadDevAllOutEp_itr(pdev);//读取全部端点的中断值
  while ( ep_intr )//一个端点一个端点的来检查那个端点有中断，这个会不会太傻了
  {
    if (ep_intr&0x1)
    {
      
      doepint.d32 = USB_OTG_ReadDevOutEP_itr(pdev, epnum);//读出该端点的具体引发中断信息
      
      /* Transfer complete */
      if ( doepint.b.xfercompl )
      {
        /* Clear the bit in DOEPINTn for this interrupt */
        CLEAR_OUT_EP_INTR(epnum, xfercompl);//传输完成，清除该中断信息，
        if (pdev->cfg.dma_enable == 1)
        {
        ...
        /* Inform upper layer: data ready */
        /* RX COMPLETE */
        //USBD_DCD_INT_fops->DataOutStage(pdev , epnum);//调用类的定义函数
        HAL_PCD_DataOutStageCallback(pdev , epnum);
        if (pdev->cfg.dma_enable == 1)
        {
          if((epnum == 0) && (pdev->dev.device_state == USB_OTG_EP0_STATUS_OUT))
          {
            /* prepare to rx more setup packets */
            //printf("USB_OTG_EP0_STATUS_OUT\r\n");
            USB_OTG_EP0_OutStart(pdev);
          }
        }        
      }
      ...
      if ( doepint.b.setup )
      {
        ...
        /* SETUP COMPLETE */
        //USBD_DCD_INT_fops->SetupStage(pdev);//控制命令数据包
        HAL_PCD_SetupStageCallback(pdev);
