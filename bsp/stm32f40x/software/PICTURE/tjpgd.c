...
{ 
    u32  rb; //读取到的字节数
    s32 fd=*(u32*)jd->device;  //待解码的文件的信息，使用FATFS中的FIL结构类型进行定义
    if(buf)     				//读取数据有效，开始读取数据
    { 
        rb = read(fd,buf,num);//调用FATFS的f_read函数，用于把jpeg文件的数据读取出来
        return rb;        		//返回读取到的字节数目
    }
    else 
    return (lseek(fd,num,SEEK_CUR) != -1) ? num:0;//重新定位数据点，相当于删除之前的n字节数据 
}  
//采用填充的方式进行图片解码显示
//jd:储存待解码的对象信息的结构体
...
{  
	...
	if(res==0)
	{
		...
		if(fd >= 0)//打开文件成功
		{ 
			res=jd_prepare(jpeg_dev,jpeg_in_func,jpg_buffer,JPEG_WBUF_SIZE,(void*)&fd);//执行解码的准备工作，调用TjpgDec模块的jd_prepare函数
